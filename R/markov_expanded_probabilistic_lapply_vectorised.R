...
#' @return Output
#' @export
markov_expanded_lapply_vectorise <- function(){
  set.seed(14143)
 ...
 # cohort.vectors[,,1,paste("State",c(2:n.states))]<-0
  ...
  lapply(c(1:n.treatments), function(i.treatment){
     ...
     #   cohort.vectors_tr_sample[i.cycle,] <- cohort.vectors_tr_sample[i.cycle-1,] %*% transition.matrices_tr_sample
        ...
        for(i.state in 1:n.states)
        {
          ...
          cohort.vectors_tr[ ,i.cycle,i.state]<- 
            rowSums(cohort.vectors_tr[ ,i.cycle-1, ] * transition.matrices_tr[,,i.state])
        }
      }
    ...
    for(i.sample in 1:n.samples)
    {
      ...
      cycle.costs_tr[i.sample,] <- cohort.vectors_tr[i.sample,,]%*%state.costs[i.sample,]
      cycle.qalys_tr[i.sample,] <- cohort.vectors_tr[i.sample,,]%*%state.qalys[i.sample,]
...
system.time(output<-markov_expanded_lapply_vectorise())
